...
dataFile <- file("C:/Users/dhana/Documents/IDS 575/HW_2/articles.train", "r")
...
# Read all individual lines in a text file.
# m = the number of training examples
testFile <- file("C:/Users/dhana/Documents/IDS 575/HW_2/articles.test", "r")
testLines <- readLines(testFile)
m <- length(testLines)
close(testFile)

# Split every string element by tokenizing space and colon.
testTokens = strsplit(testLines, "[: ]")

# Extract every first token from each line as a vector of numbers, which is the class label.
Y_test = sapply(testTokens, function(example) {as.numeric(example[1])})

# Extract the rest of tokens from each line as a list of matrices (one matrix for each line)
# where each row consists of two columns: (feature number, its occurrences)
X_list_test = lapply(testTokens, function(example) {n = length(example) - 1; matrix(as.numeric(example[2:(n+1)]), ncol=2, byrow=T)})
